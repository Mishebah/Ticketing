...
//require_once "logger/AppLogger.php";
...
class BenchMark
{
     ...
     */
    function __construct($sessionID,$settings) {
        $this->limit = array();
        $this->sessionID = $sessionID;
        $this->log = new \AppLogger($this->sessionID);
    ...
    public function logTAT($level, $method, $uniqueid = -1) {
        ...
        switch ($level) {
            ...
            DEFAULT:
                $this->log->errorLog(
                     $this->settings['TAT_ERROR'], $uniqueid,
    ...
    private function logFunctionTAT($level, $method, $uniqueid) {
        try {
            $this->log->infoLog(
                $this->settings['TAT_INFO'], $uniqueid,
