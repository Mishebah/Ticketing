...
use app\models\ReportTypesSearch;
...
class ReportTypesController extends Controller
{
    ...
    {
						if(!PermissionUtils::checkModuleActionPermission("ReportTypes",PermissionUtils::VIEW_ALL)){
    ...
    {
				   if(!PermissionUtils::checkModuleActionPermission("ReportTypes",PermissionUtils::VIEW_ONE)){
				throw new ForbiddenHttpException('You are not allowed to perform this action.');
        }
    ...
    {
								   if(!PermissionUtils::checkModuleActionPermission("ReportTypes",PermissionUtils::CREATE)){
				throw new ForbiddenHttpException('You are not allowed to perform this action.');
        }   
...
$model->insertedBy= yii::$app->user->identity->userID;
	 ...
	 $model->active = StatusCodes::ACTIVE;
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            return $this->redirect(['view', 'id' => $model->reportTypeID]);
        } else {
    ...
    {
       			if(!PermissionUtils::checkModuleActionPermission("ReportTypes",PermissionUtils::UPDATE)){
				throw new ForbiddenHttpException('You are not allowed to perform this action.');
        }       
	  $model = $this->findModel($id);
      $model->updatedBy=  yii::$app->user->identity->userID;
    ...
    {
   if(!PermissionUtils::checkModuleActionPermission("ReportTypes",PermissionUtils::DELETE)){
				throw new ForbiddenHttpException('You are not allowed to perform this action.');
        }	
      $model = $this->findModel($id);
      $model->updatedBy=  yii::$app->user->identity->userID;
	 ...
	 if($model->save())
	 	Yii::$app->session->addFlash( 'success',"ReportTypes has been deleted " );
		else
		Yii::$app->session->addFlash( 'error',"Error on deleting ReportTypes " );
